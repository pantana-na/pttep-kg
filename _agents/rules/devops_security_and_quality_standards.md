...
## Core Mandate
In addition to Spec-Driven Development (SDD), all software engineering, repository management, CI/CD pipeline automation, and cloud deployments must strictly adhere to the following 9 fundamental rules:
...
## Rule 3: Pre-Build & Pre-Deploy SAST (Static Application Security Testing via Code Mender)
1. **Mandatory SAST:** Static Application Security Testing must be executed prior to build and deployment to detect source-code vulnerabilities, unsafe dependencies, injection risks, and secret leaks.
2. **Tooling & Code Mender (`cm` CLI):**
   - Use Google Cloud **Code Mender (`cm`)** as the primary SAST and auto-remediation tool:
     - `cm scan .`: Run static security analysis across the codebase and infrastructure configs.
     - `cm mend`: Automatically generate and apply AI remediation patches for detected security issues.
3. **Zero High/Critical Vulnerabilities:** Builds and deployments must fail if unaddressed High or Critical security vulnerabilities are detected in application code.
