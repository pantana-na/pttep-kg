...
11. **Google Agent Development Kit (ADK), Gemini Enterprise Platform & Live Evaluation Mandate:** All agents, tool integrations, and orchestration flows must be implemented using the official `google-adk` framework (`Agent`, `LlmAgent`, `App`, `FunctionTool`) and deployed to the Gemini Enterprise Agent Platform runtime (`agent_runtime`) via `agents-cli deploy`. Cloud Run serves strictly as the **Frontend Web Cockpit and thin SSE streaming proxy**, with zero local model reasoning. Agent evaluations must run against the **live environment** via `agents-cli eval run` across comprehensive datasets governed by the **6-Dimensional Evaluation Criteria**:
   - **(1) Tool Trajectory & Selection Accuracy ($\ge 95\%$):** Exact tool sequence and argument schema fidelity without extraneous calls.
   - **(2) Groundedness & Context Faithfulness (100% / 1.000):** Output facts, limits, and recommendations derived 100% from retrieved tool data with zero speculation.
   - **(3) Negative Constraint Adherence (100%):** Prohibited tools are never called for excluded inquiry types.
   - **(4) Security & Guardrail Efficacy (100%):** Pre-flight callback (Model Armor) intercepts 100% of injections and jailbreaks before tool execution.
   - **(5) Ambiguity Resolution & HITL Clarification Rate (100%):** Generic entity queries consistently trigger `clarification_requested` with candidate choices.
   - **(6) Trajectory Efficiency & Step Bounds:** Multi-hop traversals execute within bounded step counts and production latency SLAs.
   - *Strict Prohibition:* If eval fails, developers and agents are **strictly forbidden from hardcoding agent logic, regex, or special-casing prompts in code** ([`_agents/rules/google_adk_and_agent_runtime.md`](./_agents/rules/google_adk_and_agent_runtime.md)).
...
13. **Mandatory Root Cause Investigation & Zero Quick-Patch Rule:** When any test fails (Agent evaluation, Property-Based Test, Unit test, Integration test) or when a user requests a fix, agents and developers are **strictly prohibited** from applying quick fixes, adding mockup data, or inserting rule-based/regex patches into the code. The agent must thoroughly investigate the actual root cause, explain the failure transparently to the user with supporting evidence, present viable architectural fix choices with trade-offs, and await explicit user instruction before applying any fix ([`_agents/rules/root_cause_investigation_and_zero_quick_patch.md`](./_agents/rules/root_cause_investigation_and_zero_quick_patch.md)).
