...
11. **Google Agent Development Kit (ADK), Gemini Enterprise Platform & Live Evaluation Mandate:** All agents, tool integrations, and orchestration flows must be implemented using the official `google-adk` framework (`Agent`, `LlmAgent`, `App`, `FunctionTool`) and deployed to the Gemini Enterprise Agent Platform runtime (`agent_runtime`) via `agents-cli deploy`. Cloud Run serves strictly as the **Frontend Web Cockpit and thin SSE streaming proxy**, with zero local model reasoning. Agent evaluations must run against the **live environment** via `agents-cli eval run` across comprehensive datasets assessing tool trajectory accuracy, groundedness, negative constraints, security efficacy, and HITL ambiguity resolution without hardcoding logic on failure ([`_agents/rules/google_adk_and_agent_runtime.md`](./_agents/rules/google_adk_and_agent_runtime.md)).
